...
function TimestampBadge({ entry, className = "" }: TimestampDisplayProps) {
  ...
  return (
    <div className={`timestamp-entry inline-flex items-center space-x-2 px-3 py-1 rounded-md border text-sm font-medium ${getTimestampColor(entry.type)} ${className}`}>
      <Icon 
        icon={getIcon(entry.type)} 
        className="w-3 h-3" 
      />
      <span className="font-mono text-xs">
        {getTimestampLabel(entry.type)}:
      </span>
      <span>
  ...
  return (
    <div className={`timestamps-display flex flex-wrap gap-2 mb-3 ${className}`}>
      {entries.map((entry, index) => (
        <TimestampBadge 
          key={`${entry.type}-${entry.lineNumber}-${index}`} 
          entry={entry}
        />
      ))}
    </div>
